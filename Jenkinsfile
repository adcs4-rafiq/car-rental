        ...
        stage('Deploy to Azure VM') {
            steps {
                sshagent(['azure-ssh-key']) {
                    sh '''
                        scp -o StrictHostKeyChecking=no deploy/docker-compose.prod.yml ${VM_USER}@${VM_HOST}:~/docker-compose.prod.yml
                        scp -o StrictHostKeyChecking=no deploy/deploy.sh ${VM_USER}@${VM_HOST}:~/deploy.sh
